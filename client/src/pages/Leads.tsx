...
export function Leads() {
  ...
  const handleLeadCreated = (lead: any) => {
    if (!lead || !lead.id) {
      // Silent error - just refresh the list
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={<MapPin className="w-12 h-12 text-gray-400" />}
          title={searchQuery || statusFilter ? "No leads match your filters" : "No leads yet"}
          description={searchQuery || statusFilter
            ? "Try adjusting your search or filters to see more results"
            : "Add your first lead by typing a property address above. Our AI will analyze it and provide insights automatically."
          }
          action={!searchQuery && !statusFilter ? (
            <div className="mt-4 text-sm text-gray-500 dark:text-gray-400">
              <p>💡 Tip: Try entering an address like "123 Main St, Los Angeles, CA"</p>
            </div>
          ) : undefined}
        />
      ) : (
