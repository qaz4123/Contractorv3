...
export function LeadDetail() {
  ...
  const convertToProjectMutation = useMutation({
    mutationFn: async () => {
      // Get current lead data from query cache to ensure we have latest
      const currentLead = queryClient.getQueryData<Lead>(['lead', id]);
      ...
      if (!response.data.success) {
        throw new Error(response.data.error || 'Failed to convert lead to project');
      }
